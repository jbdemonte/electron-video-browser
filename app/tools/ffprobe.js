...
const ffprobeBinaryPath = join(__dirname, '..', 'bin', 'ffprobe');
...
function resumeResponse(response) {
  ...
  const video = streams.filter((stream) => stream.codec_type === 'video' && stream.codec_name !== 'mjpeg').shift();

  if (!video) {
    throw new Error('Video stream is missing');
  }

  const filename = basename(format.filename);
...
export default async function extractVideoInformations(source) {
  ...
  try {
    const response = await ffprobeExecFile('-show_streams', '-show_format', '-print_format', 'json', source);
    release();
